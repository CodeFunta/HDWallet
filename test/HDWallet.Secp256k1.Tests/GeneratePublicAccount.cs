...
namespace HDWallet.Secp256k1.Tests
{
    public class GeneratePublicAccount
    {
        ...
        public void ShouldCreatePublicAccountFromMasterKey()
        {
            ...
            ClassicAssert.AreEqual("0374c393e8f757fa4b6af5aba4545fd984eae28ab84bda09df93d32562123b7a1c", depositWallet0.PublicKeyBytes.ToHexString());

            // m/44'/0'/0'/0/1
            var depositWallet1 = publicAccount.GetExternalPublicWallet(1);
            ClassicAssert.AreEqual("025166e4e70b4ae6fd0deab416ab1c3704f2aa5dbf451be7639ca48fe6d273773c", depositWallet1.PublicKeyBytes.ToHexString());
        }
        ...
        public void ShouldMatchAccountsFromXprixOrXpub()
        {
            ...
            ClassicAssert.AreEqual(publicAccount.GetExternalPublicWallet(0).PublicKeyBytes, accountHDWallet.GetExternalWallet(0).PublicKeyBytes);
            ClassicAssert.AreEqual(publicAccount.GetExternalPublicWallet(1).PublicKeyBytes, accountHDWallet.GetExternalWallet(1).PublicKeyBytes);

            ClassicAssert.AreEqual(publicAccount.GetInternalPublicWallet(0).PublicKeyBytes, accountHDWallet.GetInternalWallet(0).PublicKeyBytes);
            ClassicAssert.AreEqual(publicAccount.GetInternalPublicWallet(1).PublicKeyBytes, accountHDWallet.GetInternalWallet(1).PublicKeyBytes);
        }
    }
}